...
- The `engagementReady` gate that blocks the Dashboard for clients without synchronized AssurePro data was not applied to the Planning routes — switching to any non-Riverside client and navigating directly to Materiality, Risk Assessment or any other Planning URL still rendered Riverside's real figures under the new client's name. Added the same gate to `PlanningShell` so every Planning route is honestly blocked for a client whose data hasn't been set up, not just the Dashboard.
- Chart hover tooltips (Dashboard donut and bar charts, Materiality benchmark trend) were absent again — hovering showed nothing. Re-added a compact, card-bounded tooltip (and a center-label swap for the donut) that stays inside its container instead of floating freely, since an unconstrained floating tooltip was the original reason they were removed.
- The Guide button's fixed position again visually overlapped the sticky Planning action bar and could swallow clicks meant for "Complete step"/"Complete materiality"; the mobile nav drawer again didn't close on Escape or outside click; the sidebar's Fieldwork item again showed a lock icon regardless of `state.locked`; and the Entity & Controls step list again lost its scroll-affordance fade at tablet widths. All four were fixed in a prior pass and are re-applied here.
- An unknown/mistyped Planning step slug again rendered a blank content area instead of a "step doesn't exist" fallback; re-added the guard.
- Note for future revisions: several of the items above are the second or third time they've been fixed in this codebase, each time reverted by a subsequent externally-authored revision that branched from an older base rather than the latest commit. Worth basing future audit passes on the latest `main` rather than a stored snapshot, to avoid re-litigating the same fixes.
