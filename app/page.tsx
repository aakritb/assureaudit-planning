  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-area">
        ...
        ) : path === "/engagements" ? <Engagements navigate={navigate} update={update} state={state} /> : path.startsWith("/engagement/") ? <EngagementHome navigate={navigate} state={state} /> : <Dashboard navigate={navigate} state={state} />}
      </main>
...
function Sidebar({ path, navigate, state, update, mobileNav, setMobileNav }: { path: string; navigate: (p: string) => void; state: DemoState; update: (p: Partial<DemoState>, m?: string) => void; mobileNav: boolean; setMobileNav: (v: boolean) => void }) {
  ...
  return <aside className={`sidebar ${mobileNav ? "mobile-open" : ""}`} style={{transform: mobileNav ? "none" : undefined}}>
    ...
    <nav>
      ...
      <button className="nav-item" onClick={() => navigate("/engagement/bbawc/planning")}><ClipboardCheck/><span>My work</span><span className="nav-count">{attentionItems(state).length}</span></button>
...
function Dashboard({ navigate, state }: { navigate: (p: string) => void; state: DemoState }) {
  ...
  return <div className="page dashboard-page">
    ...
    <section className="continue-audit">
      <div className="continue-icon">{declined?<AlertCircle/>:<ClipboardCheck/>}</div>
      <div><span>{declined?"Engagement declined":"Continue where you left off"}</span><h2>Planning</h2><p>{declined?"Acceptance & continuance concluded Decline — Planning does not proceed to Fieldwork.":`${pct}% complete${next?` · Next: ${next.title} — ${next.detail}`:" · All phases complete"}`}</p></div>
      <div className="continue-progress"><span><strong>{pct}%</strong> complete</span><i><em style={{width:`${100-pct}%`}}/></i></div>
      <button className="primary-btn" onClick={() => navigate(declined?"/engagement/bbawc/planning/setup":`/engagement/bbawc/planning/${next?next.route:"review"}`)}>{declined?"Review decision":next?`Open ${next.title}`:"Go to review"} <ArrowRight/></button>
    </section>
...
function Engagements({ navigate, update, state }: { navigate: (p: string) => void; update: (p: Partial<DemoState>, m?: string) => void; state: DemoState }) {
  ...
  const rows = [
    { name: "Brooklyn Bridge Animal Welfare Coalition, Inc.", planning: bbawcPlanning, live: true },
    ...
    <div className="table-card"><div className="table-toolbar"><div className="search"><Search/><input placeholder="Search engagements"/></div><div className="topbar-popover"><button className={`filter-btn ${statusFilter !== "All" ? "active" : ""}`} onClick={() => setFilterOpen(!filterOpen)}><Filter/>Filters{statusFilter !== "All" && <i className="filter-badge"/>}</button>{filterOpen && <div className="dropdown-menu filter-menu">
      ...
      <table><thead><tr><th>Client</th><th>Engagement</th><th>Period</th><th>Planning</th><th>Assigned team</th><th></th></tr></thead><tbody>
        ...
        {rows.map(r => r.live ? <tr key={r.name}><td><strong>{r.name}</strong><span>Nonprofit</span></td><td>Financial Audit</td><td>Dec 31, 2025</td><td><span className={`status-pill ${bbawcPillClass}`}>{bbawcPlanning}</span><StatusCountBadges compact complete={bbawcCounts.complete} inProgress={bbawcCounts.inProgress} attention={bbawcCounts.attention}/></td><td><div className="avatar-stack"><i>JA</i><i>MK</i><i>OO</i></div></td><td><button className="icon-btn" onClick={() => navigate("/engagement/bbawc")}><ChevronRight/></button></td></tr>
          : <tr key={r.name}><td><strong>{r.name}</strong><span>Nonprofit</span></td><td>Financial Audit</td><td>Jun 30, 2025</td><td><span className="status-pill approved">Approved</span></td><td><div className="avatar-stack"><i>RP</i><i>MK</i></div></td><td><button className="icon-btn" onClick={() => update({}, "This demo only includes one fully wired engagement — Brooklyn Bridge Animal Welfare Coalition.")}><ChevronRight/></button></td></tr>)}
      </tbody></table>
    </div>
...
function EngagementHome({ navigate, state }: { navigate: (p: string) => void; state: DemoState }) {
  ...
  return <div className="page"><div className="breadcrumbs"><button onClick={() => navigate("/dashboard")}>Dashboard</button><ChevronRight/><span>Financial Audit</span></div>
    ...
    <div className="section-card"><div className="section-title"><h2>Engagement timeline</h2><button className="text-link" onClick={() => navigate("/engagement/bbawc/planning/audit-trail")}>View audit trail</button></div><div className="timeline"><div className={declined?"declined-step":foundationDone?"done":"active"}>{declined?<AlertCircle/>:foundationDone?<Check/>:<Clock3/>}Acceptance</div><div className={dataDone?"done":foundationDone&&!declined?"active":""}>{dataDone?<Check/>:<Clock3/>}Data ingested</div><div className={state.locked?"done":!declined?"active":""}>{state.locked?<Check/>:<Clock3/>}Planning</div><div><LockKeyhole/>Fieldwork</div><div><Circle/>Reporting</div></div></div>
  </div>;
}
...
function PlanningOverview({ state, update, navigate }: { state: DemoState; update:(p:Partial<DemoState>,m?:string)=>void; navigate:(p:string)=>void }) {
  ...
  const nextAction=declined?{title:"Engagement declined",detail:"Acceptance & continuance concluded Decline. No further Planning step should be completed; change the decision in Engagement Foundation to resume.",cta:"Review acceptance decision",route:"setup"}:openPhase?{title:`Continue ${openPhase.title}`,detail:openPhase.detail,cta:"Open "+openPhase.title,route:openPhase.route}:{title:"All planning phases complete",detail:"Submit for Manager and Partner review when ready.",cta:"Go to Publish & Approval",route:"publish"};
...
function SetupView({ state, update }: { state:DemoState; update:(p:Partial<DemoState>,m?:string)=>void }) {
  ...
  return <div className="content-pad"><div className="subtabs">{tabs.map(t=><button key={t} className={tab===t?"active":""} onClick={()=>setTab(t)}>{t}{tabsDone[t]&&<Check size={13}/>}</button>)}</div>
    ...
    {tab==="Engagement details" && <FormSection update={update} title="Engagement details & terms" subtitle="Core engagement facts are synchronized from setup."><div className="form-grid"><Field label="Entity"><input value="Brooklyn Bridge Animal Welfare Coalition, Inc." readOnly/></Field><Field label="Period end"><input value="December 31, 2025" readOnly/></Field><Field label="Reporting framework"><select><option>US GAAP</option></select></Field><Field label="Audit type"><select><option>Financial Audit</option></select></Field><Field label="Industry"><select><option>Nonprofit / Animal welfare</option></select></Field><Field label="Reporting deadline"><input type="date" defaultValue="2026-04-30"/></Field></div><CheckRow title="Group audit" detail="Enable component scoping and component materiality"/></FormSection>}
...
function EntityControls({ state, update }: { state:DemoState; update:(p:Partial<DemoState>,m?:string)=>void }) {
  ...
  return <div className="content-pad"><QuestionnaireWorkspace state={state} update={update}/><div className="entity-layout"><aside className="entity-nav">{sections.map((s,i)=><button className={section===s?"active":""} onClick={()=>setSection(s)} key={s}><span>{i<3?<CheckCircle2/>:<Circle/>}</span><div><strong>{s}</strong><small>{i<3?"Validated":"In progress"}</small></div><ChevronRight/></button>)}</aside><div className="entity-content">
    ...
    {section==="Understanding the entity" && <><Question number="01" title="Describe the organization’s business model and primary revenue streams." tag="Validated"><textarea defaultValue="The organization provides animal rescue, shelter, adoption and community veterinary services. Revenue is primarily derived from public contributions, restricted grants, adoption fees and fundraising events."/><Evidence update={update}/><AiDraft/></Question><Question number="02" title="What significant contracts or unusual transactions occurred during the period?" tag="Needs validation"><textarea defaultValue="A new three-year city grant began in Q4 with performance conditions tied to animals placed and outreach milestones."/><Evidence file="2025 Grant Agreement.pdf" pages="1–6" update={update}/></Question></>}
...
function PublishPreviewModal({ title, state, onClose }: { title:string; state:DemoState; onClose:()=>void }) {
  ...
  return <div className="modal-backdrop"><div className="modal">
    ...
    {title==="Planning memo" && <div className="preview-body">
      <InfoBlock label="Entity" text="Brooklyn Bridge Animal Welfare Coalition, Inc. · Financial Audit · Period ended December 31, 2025"/>
...
function PlanningManager({state,update,navigate}:{state:DemoState;update:(p:Partial<DemoState>,m?:string)=>void;navigate:(p:string)=>void}) {
  ...
  return <section className="planning-manager">
    ...
    {view==="Summary"&&(()=>{const attentionParts=attentionItems(state);const attentionCount=attentionParts.length;const phaseCounts=phaseStatusCounts(state);return <div className="manager-summary"><div className="key-information"><div><span>Revenue</span><strong>$9.6M</strong><small className="up">+5.5% vs prior year</small></div><div><span>Net assets</span><strong>$2.21M</strong><small>−2.8% vs prior year</small></div><div><span>Overall materiality</span><strong>{money(state.materialityOverride?240000:state.benchmark*(state.materialityPct/100))}</strong><small>{state.materialityPct}% of revenue</small></div><div><span>Entity risk</span><strong>{state.entityRisk}</strong><small>4 documented drivers</small></div></div><StatusCountBadges complete={phaseCounts.complete} inProgress={phaseCounts.inProgress} attention={phaseCounts.attention}/><div className="manager-attention"><div>{attentionCount>0?<AlertCircle/>:<CheckCircle2/>}<span><strong>{attentionCount>0?`${attentionCount} item${attentionCount===1?"":"s"} need${attentionCount===1?"s":""} attention`:"Nothing needs attention"}</strong>{attentionCount>0&&<small>{attentionParts.join(" · ")}</small>}</span></div><div className="team-line"><span><i className="person-avatar violet">JA</i>Preparer</span><span><i className="person-avatar blue">MK</i>Manager</span><span><i className="person-avatar green">OO</i>Partner</span></div></div></div>})()}
...
function NewEngagementWizard({ onClose, update }: { onClose: () => void; update: (p: Partial<DemoState>, m?: string) => void }) {
  ...
  const create = () => { onClose(); update({}, `"${client}" created — this demo scopes fully wired functionality to Brooklyn Bridge Animal Welfare Coalition, Inc.`); };
