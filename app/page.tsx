...
const CLIENTS: ClientRecord[] = [
  {slug:"bbawc",initials:"RY",name:"Riverside Youth & Family Services, Inc.",industry:"Nonprofit",subIndustry:"Youth & family services",auditType:"Financial Audit",period:"Dec 31, 2025",stage:"Workpapers",progress:63,openItems:4,owner:"Jasmine Alvarez",due:"Aug 18",ready:true},
  {slug:"harbor",initials:"HC",name:"Harbor Community Foundation",industry:"Nonprofit",subIndustry:"Community foundation",auditType:"Financial Audit",period:"Jun 30, 2025",stage:"Workpapers",progress:42,openItems:2,owner:"Meera Kapoor",due:"Aug 21",ready:true},
  {slug:"greenfield",initials:"GH",name:"Greenfield Housing Alliance",industry:"Real estate",subIndustry:"Affordable housing",auditType:"NFP Audit",period:"Sep 30, 2025",stage:"Data ingest",progress:28,openItems:6,owner:"Leo Chen",due:"Aug 16",ready:true},
  {slug:"metro",initials:"MA",name:"Metro Arts Council",industry:"Nonprofit",subIndustry:"Arts & culture",auditType:"Fund Audit",period:"Jun 30, 2025",stage:"Setup required",progress:0,openItems:1,owner:"Unassigned",due:"Not set",ready:false},
  {slug:"horizon",initials:"HR",name:"Horizon Retirement Plan",industry:"Employee benefits",subIndustry:"Defined contribution plan",auditType:"EBP Audit",period:"Dec 31, 2025",stage:"Review",progress:81,openItems:3,owner:"Oscar Owner",due:"Aug 14",ready:true},
  {slug:"cedar",initials:"CG",name:"Cedar Grove Outreach",industry:"Nonprofit",subIndustry:"Human services",auditType:"Financial Audit",period:"Mar 31, 2025",stage:"Complete",progress:100,openItems:0,owner:"Meera Kapoor",due:"Complete",ready:true},
];
...
function Sidebar({ path, navigate, state, update, mobileNav, setMobileNav }: { path: string; navigate: (p: string) => void; state: DemoState; update: (p: Partial<DemoState>, m?: string) => void; mobileNav: boolean; setMobileNav: (v: boolean) => void }) {
  ...
  const [ingestOpen,setIngestOpen]=useState(inIngest); const [productOpen,setProductOpen]=useState(false);
  ...
  return <aside className={`sidebar ${mobileNav ? "mobile-open" : ""}`} style={{transform: mobileNav ? "none" : undefined}}>
    ...
    <nav>
      ...
      <button className={`nav-item ${path === "/my-work" ? "active" : ""}`} onClick={() => navigate("/my-work")}><ClipboardCheck/><span>Workflow</span><span className="nav-count">{myWorkBadgeCount(state)}</span></button>
      {!clientContext&&<><p className="nav-label branch-label">Client workspace</p><div className="client-workspace-nav workspace-preview"><button className="select-client-nav" onClick={()=>navigate("/clients")}><Search/><span>Select a client</span><ChevronRight/></button><button disabled><HomeIcon/><span>Overview</span><LockKeyhole className="branch-lock"/></button><button disabled><Database/><span>Data ingest</span><LockKeyhole className="branch-lock"/></button><button disabled><ClipboardCheck/><span>Planning</span><LockKeyhole className="branch-lock"/></button><button disabled><Search/><span>Fieldwork</span><LockKeyhole className="branch-lock"/></button><button disabled><FileCheck2/><span>Completion</span><LockKeyhole className="branch-lock"/></button></div></>}
      {clientContext&&<><p className="nav-label branch-label">Client workspace</p><button className="sidebar-client-chip" onClick={()=>navigate(`/clients/${clientSlug}`)} title={`${client.name} — back to overview`}><i>{client.initials}</i><span><strong>{client.name}</strong><small>{client.industry}</small></span></button><div className="client-workspace-nav"><button className={path===`/clients/${clientSlug}`||path===`/engagement/${clientSlug}`?"active":""} onClick={()=>navigate(`/clients/${clientSlug}`)}><HomeIcon/><span>Overview</span></button>{!client.ready?<button className="setup-blocked-nav" disabled><AlertTriangle/><span>Finish setup in AssurePro</span><LockKeyhole className="branch-lock"/></button>:<><button className={`branch-parent ${inIngest?"active":""}`} onClick={()=>{setIngestOpen(!ingestOpen);if(!inIngest)navigate(`/engagement/${clientSlug}/ingest/details`)}}><Database/><span>Data ingest</span><b>{client.progress}%</b><ChevronDown className={ingestOpen?"rotated":""}/></button>{ingestOpen&&<div className="branch-children ingest-branch">{ingestSteps.map((label,i)=><button key={label} className={active===label.toLowerCase().replace(" ","-")?"active":""} onClick={()=>navigate(`/engagement/${clientSlug}/ingest/${label.toLowerCase().replace(" ","-")}`)}><i className={i<4?"approved":i===4?"warning":""}/><span>{label}</span></button>)}</div>}{/* Materiality is intentionally not listed here — it lives only in Data Ingest; landing on /planning/materiality (e.g. a stale link) shows a banner pointing there instead of a dead sidebar entry. Planning's own workpaper board and insight rail already link to every sub-view (setup, entity-controls, risks, responses, publish), so this is a single entry rather than a partial duplicate list. */}<button className={inPlanning?"active":""} onClick={()=>navigate(`/engagement/${clientSlug}/planning`)}><ClipboardCheck/><span>Planning</span><b>{planningProgressPct(state)}%</b></button><button className={inFieldwork?"active":""} onClick={()=>navigate(`/engagement/${clientSlug}/fieldwork`)}><Search/><span>Fieldwork</span>{state.fieldworkSynced&&<b>Synced</b>}</button><button onClick={()=>update({},"Completion unlocks after Fieldwork")}><FileCheck2/><span>Completion</span><LockKeyhole className="branch-lock"/></button></>}</div></>}
...
function Topbar({ path, state, update, navigate, onMenu, demoOpen, setDemoOpen }: { path:string; state: DemoState; update: (p: Partial<DemoState>, m?: string) => void; navigate: (p: string) => void; onMenu: () => void; demoOpen: boolean; setDemoOpen: (v: boolean) => void }) {
  ...
  const searchItems = [
    ...
    { kind:"Page", title:"Workflow", detail:"Assigned tasks, approvals and reviews", route:"/my-work", icon:ClipboardCheck },
...
function MyWork({ navigate, state, update }: { navigate:(p:string)=>void; state:DemoState; update:(p:Partial<DemoState>,m?:string)=>void }) {
  ...
  return <div className="page my-work-page"><div className="page-heading"><div><p className="eyebrow">Synced from AssurePro</p><h1>Workflow</h1><p>Tasks flow from AssurePro's Workflow module and stay in sync — edits here update AssurePro too.</p></div>{view!=="Board"&&<div className="my-work-summary"><span><strong>{urgentCount}</strong> urgent</span><span><strong>{blockedCount}</strong> blocked</span></div>}</div>
...
function Dashboard({ navigate, state }: { navigate: (p: string) => void; state: DemoState }) {
  ...
  const dataIngestCount=CLIENTS.filter(c=>c.stage==="Data ingest").length;
  return <div className="page firm-dashboard">
    ...
    <section className="portfolio-kpis"><article><span>Active engagements</span><strong>6</strong><small>Across 6 clients</small></article><article><span>Need attention</span><strong>4</strong><small>2 due today</small></article><article><span>In data ingest</span><strong>{dataIngestCount}</strong><small>{dataIngestCount===1?"1 client":`${dataIngestCount} clients`} still uploading source files</small></article><article><span>Awaiting review</span><strong>1</strong><small>Partner review due Aug 14</small></article></section>
...
function clientCell(c:ClientRecord,key:string){
  ...
  if(key==="documents")return <span key={key}>{docsForClient(c).length}</span>;
...
const AUDIT_STAGE_ORDER=["Data ingest","Planning","Fieldwork","Completion"];
const CATEGORY_STAGE:Record<string,string>={"Data ingest":"Data ingest","Planning":"Planning","Financial Reporting":"Completion"};
  ...
  return <div className="page client-overview-page">
    ...
    <div className="client-overview-grid-v2">
      ...
      <aside className="client-stat-cards">
        ...
        <div className="stat-card-row"><button className="stat-card compact" onClick={()=>setTab("Engagements")}><strong>{engagements.length}</strong><span>Engagements</span></button><button className="stat-card compact" onClick={()=>setTab("Documents")}><strong>{docsForClient(client).length}</strong><span>Documents</span></button></div>
...
function EngagementHome({ navigate, state }: { navigate: (p: string) => void; state: DemoState }) {
  ...
  const journey=[
    ...
    {title:"Completion",detail:"Begins after fieldwork",date:"Apr 30",state:"locked",route:"planning",progress:0},
  ];
...
function GlobalGuide({path,open,setOpen}:{path:string;open:boolean;setOpen:(v:boolean)=>void;navigate:(p:string)=>void}){
  ...
  const guide=path==="/dashboard"?{title:"Firm dashboard",steps:[
  ...
  ]}:path==="/my-work"?{title:"Workflow",steps:[
    ...
    ["Follow the workflow","Data Ingest prepares source data before Workpapers, Fieldwork and Completion."],
...
const ENGAGEMENT_PHASES = ["Data ingest", "Planning workpapers", "Fieldwork", "Completion"];
