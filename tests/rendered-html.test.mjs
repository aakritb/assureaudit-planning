...
test("keeps the AssureAudit application shell and focused dashboard", async () => {
  ...
  assert.doesNotMatch(page, /className="client-command-menu"/);
  assert.doesNotMatch(page, /Search client, audit type or period/);
  assert.doesNotMatch(page, /aria-label="Switch client"/);
  ...
  assert.match(page, /function AssureProPanel/);
  assert.match(page, /const openEngagement=/);
